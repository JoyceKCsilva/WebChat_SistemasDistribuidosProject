...
class MainApp {
  ...
  bindEvents() {
    ...
    document
      .getElementById("enter-created-room-btn")
  ...
  renderRooms() {
    ...
    roomsList.innerHTML = this.rooms
      .map((room) => {
        const isOwner = room.is_owner;
        const roomCode = room.room_code;
        const roomName = this.escapeHtml(room.room_name);
        const createdDate = new Date(room.created_at).toLocaleDateString(
          "pt-BR"
        );

        return `
            <div class="room-card" data-room-code="${roomCode}">
                <div class="room-card-header">
                    <h3 class="room-name">${roomName}</h3>
                    ${
                      isOwner
                        ? '<span class="room-owner-badge">Dono</span>'
                        : ""
                    }
                </div>
                <div class="room-code">Código: ${roomCode}</div>
                <div class="room-info">
                    <small>Criada em: ${createdDate}</small>
                </div>
                <div class="room-actions">
                    <button class="btn-enter-wide" data-action="enter" data-room-code="${roomCode}">
                        Entrar
                    </button>
                </div>
            </div>
  ...
  bindRoomActions() {
    const roomsList = document.getElementById("rooms-list");
    console.log("bindRoomActions chamado, roomsList:", roomsList);

    // Remover listeners antigos se existirem
    const newRoomsList = roomsList.cloneNode(true);
    roomsList.parentNode.replaceChild(newRoomsList, roomsList);

    // Usar event delegation para capturar cliques em botões
    newRoomsList.addEventListener("click", (e) => {
      console.log("Clique detectado no rooms-list:", e.target);
      ...
      const roomCode = button.dataset.roomCode; // data-room-code vira roomCode no dataset

      console.log("Botão clicado:", action, roomCode);
      console.log("Dataset completo:", button.dataset);

      if (action === "enter") {
        console.log("Executando enterRoom");
        this.enterRoom(roomCode);
      }
    });

    // Adicionar clique no card para entrar na sala
    newRoomsList.addEventListener("click", (e) => {
      const roomCard = e.target.closest(".room-card");
      if (!roomCard) return;

      // Não fazer nada se o clique foi em um botão ou actions
      if (e.target.closest(".room-actions, button")) return;

      const roomCode = roomCard.dataset.roomCode; // data-room-code vira roomCode no dataset
      console.log("Clique no card da sala:", roomCode);
      this.enterRoom(roomCode);
    });

    console.log("Event listeners adicionados ao rooms-list");
  }
...
// Declarar a variável global antes do DOMContentLoaded
var mainApp;

// Função de debug para testar no console
window.testMainApp = function () {
  console.log("Testing mainApp availability:");
  console.log("mainApp:", mainApp);
  console.log("window.mainApp:", window.mainApp);
  console.log("typeof mainApp:", typeof mainApp);
  if (mainApp && mainApp.confirmCloseRoom) {
    console.log("confirmCloseRoom method:", mainApp.confirmCloseRoom);
    return "mainApp está disponível e tem o método confirmCloseRoom";
  } else {
    return "mainApp não está disponível ou não tem o método confirmCloseRoom";
  }
};
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  mainApp = new MainApp();
  // Também adicionar ao window para garantir acesso global
  window.mainApp = mainApp;
  console.log("MainApp inicializado:", mainApp);
  console.log("window.mainApp disponível:", window.mainApp);
});
